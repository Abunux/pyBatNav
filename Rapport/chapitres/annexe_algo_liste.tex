...
\newpage
\section{Détermination des bateaux coulés}
...
\tab{1}la liste de case marquées comme coulées\\
...
\tab{3}On continue la boucle (on ignore cette case)\\
\tab{2}0\sto k\\
\tab{2}Tant que case[0]+k*direction[0] < xmax et case[1]+k*direction[1] < ymax\\
\tab{3}et (case[0]+k*direction[0],case[1]+k*direction[1]) est marquée touchée :\\
...
\tab{2}\ \ ou (case[direction[1]]+k== dimensions[direction[1] \\
\tab{2}\ \ \ \ \ \ ou (case[0]+k*direction[0], case[1]+k*direction[1]) est manquée)\\
\tab{2}\ \ \ \ ) ou len(liste\_touchees)==taille\_max :\\
\tab{5}Enlever le bateau de taille len(liste\_touchees) des bateaux restants\\
...
Enfin on décide que le bateau est coulé si ses extrémités sont sur les bord de la grille ou on été marquées comme manquées. On marque alors ses cases adjacentes comme impossibles et on l'enlève de la liste des bateaux à chercher.
