...
Lors de cette phase, l'algorithme va tirer sur la case qui peut contenir le plus de bateau comme vu au chapitre \ref{chap_grille}, section \ref{opti_aveugle} avec la méthode \texttt{Ordi.make\_case\_aleatoire(self)}.
...
L'algorithme va également vérifier si le plus petit bateau ne peut pas rentre dans une direction. Imaginons, par exemple, que le plus petit bateau restant soit de taille 3 et qu'on vienne de toucher la case $(3,0)$ dans la configuration suivante :
...
\end{center}
Il ne sert alors à rien de mettre la case $(3,0)$ dans la file d'attente car le plus petit bateau de taille 3 ne peut pas rentre verticalement en $(3,0)$.
...
\medskip
 ...
 Enfin on va classer la file d'attente en ordre décroissant de nombre de bateaux possibles comme vu au chapitre \ref{chap_grille}, section \ref{opti_touche} avec la méthode \texttt{Ordi.shuffle\_queue(self)}.
